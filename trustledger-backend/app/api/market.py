...
@router.get("/live")
async def get_live_market_data(current_user: User = Depends(get_current_user)):
    live_data = []
...
@router.get("/risk")
async def get_portfolio_risk(current_user: User = Depends(get_current_user)):
    portfolio_risk = round(random.uniform(25, 65), 2)
    volatility_index = round(random.uniform(15, 55), 2)
    sentiment = "Bullish" if portfolio_risk < 35 else "Bearish" if portfolio_risk > 55 else "Neutral"
    ...
    return {
        ...
        "var_95": round(portfolio_risk * 0.15, 2),
...
async def get_market_volatility(current_user: User = Depends(get_current_user)):
    volatility_data = []
