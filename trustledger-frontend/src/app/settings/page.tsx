...
export default function Settings() {
  ...
  const handleSaveProfile = async () => {
    setSaving(true)
    try {
      await authAPI.updateSettings({ full_name: fullName, phone: phone })
  ...
  const handleUpdatePassword = async () => {
    ...
    setSaving(true)
    try {
      await authAPI.updateSettings({ current_password: currentPassword, new_password: newPassword })
      setCurrentPassword('')
      setNewPassword('')
      setConfirmPassword('')
      showSavedMessage()
    } catch (err: any) {
      alert(err.response?.data?.detail || 'Failed to update password. Please try again.')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <VoiceNavigation currentPage="Settings" />
    </div>
  )
}