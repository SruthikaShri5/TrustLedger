...
export default function Compliance() {
  ...
  const loadComplianceData = async () => {
    try {
      ...
      const [scoreRes] = await Promise.all([
        complianceAPI.getScore().catch(() => ({ data: null }))
      ])
      if (scoreRes.data) setComplianceData(scoreRes.data)
    } catch (err) {
  ...
  const handleRunCheck = async () => {
    try {
      setLoading(true)
      const res = await complianceAPI.runCheck()
  ...
  const overallScore = complianceData?.overall_score ?? 98
  const status = complianceData?.status ?? 'excellent'
  const breakdown = complianceData?.breakdown ?? {}

  const chartData = Object.keys(breakdown).length > 0
    ? Object.entries(breakdown).map(([check, score]) => ({ check, status: score as number }))
    : [
        { check: 'KYC', status: 100 },
        { check: 'AML', status: 100 },
        { check: 'PEP', status: 100 },
        { check: 'Sanctions', status: 100 },
      ]
  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-gray-900">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-600 dark:text-gray-400">
                        {complianceData?.last_check
                          ? `Last checked: ${new Date(complianceData.last_check).toLocaleDateString()}`
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 mt-2">
                    <div className="bg-green-600 h-2 rounded-full" style={{width: `${overallScore}%`}}></div>
                  </div>
                  <Button
                    size="sm"
                    variant="outline"
                    ...
                    <div className="flex items-start space-x-3">
                      ...
                      <div>
                        <p className="font-semibold text-green-900 dark:text-green-200">
                          {overallScore >= 90 ? 'Fully Compliant' : overallScore >= 70 ? 'Mostly Compliant' : 'Needs Attention'}
                        </p>
                        <p className="text-sm text-green-700 dark:text-green-300 mt-1">
                          {overallScore >= 90
                            ? 'All your documents are verified and up to date. Your account meets all regulatory requirements.'
                            : 'Some compliance items need attention. Please review and update your documents.'}
                        </p>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="flex items-start space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-yellow-700 dark:text-yellow-300 mt-1">
                          {complianceData?.next_review
                            ? `Next compliance review due: ${complianceData.next_review}`
      ...
      <VoiceNavigation currentPage="Compliance" />
    </div>
  )
}
