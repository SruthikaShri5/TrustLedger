...
export default function Reports() {
  ...
  const loadReportsData = async () => {
    try {
      ...
      const [statsRes, reportRes] = await Promise.all([
        ...
        aiAPI.generateReport().catch(() => ({ data: null }))
      ])
      ...
      if (reportRes.data?.summary) {
        ...
        const topCat = summary.categories
          ? Object.entries(summary.categories).sort(([, a], [, b]) => (b as number) - (a as number))[0]?.[0]
          : null
        ...
        setInsights({
          ...
          top_category: topCat,
          recommendation: reportRes.data.ai_insights?.[0] || null
        })
      }
    } catch (err) {
  ...
  const generateMonthlyData = () => {
    ...
    // Distribute income evenly across months
  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <VoiceNavigation currentPage="Reports" />
    </div>
  )
}
